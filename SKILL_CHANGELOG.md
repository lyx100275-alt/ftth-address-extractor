...
### 2026-09-20（九十·补）：图签单元归属消歧改按行（dy 优先）—— 小区甲 R1 实测
...
- **刻度兜底可见性（pick_scale）**：无 y 完全覆盖列而走 ③/兜底时记入清单，主循环后统一打印
  ...
  `--col-scale-map` 裁决）。小区乙全图实跑：各列均有完全覆盖列，新块静默（符合预期），
...
- **验证**：冒烟 ALL PASS（双解释器；有依赖机含 T5 + T6 gen）；`ftth.py budget` rc=0；
  小区乙 `count-box` 全图重跑：门禁、偏移/共用告警、产物均正常；改动脚本 py_compile 全过。
...
- **auto_scaled 上收（P1-4）**：analyze_coverage / count_box_icons 逐字相同 4 行并入
  ...
  小区乙 r4verify 全链重跑四产物哈希与 r3 逐键一致，零行为差。
...
### 2026-09-19（八十八）：assemble 双 P0 —— 未导入名崩溃 + 单元归一吞楼号（小区乙三轮实跑暴露，跑一轮修一轮）

- **P0-1：`assemble_households.py` NameError，全链路阻断**：`norm_unit` 用到
  `UNIT_RE_SRC` / `cn2num` / `unit_num`，但只引了 `protect_out_path` —— 任何 assemble
  调用必崩（实测小区乙 `ftth.py assemble` 直接 NameError）。全仓排查仅此一处漏引
  （plan/parse/probe/read_titleblock 均正常引入）。修复：补 import；`cn2num` 修后无引用，
  已移出 import 行。备份：`桌面/小区乙/.temp/backup_20260919_2110_pre_assemble_unitresrc/`。
...
- **验证（小区乙 .temp/iter_20260919_noref_3round/r3）**：三轮管线逐键一致（parsed/coverage/
...
### 2026-09-19（八十二）：边界线加固四项 —— 分带机制上收唯一实现 + 列共识容差旋钮 + 单锚点取证
   **背景**：边界线主线遗留 4 个加固项（②列共识 ndigits=6 微差分裂 / ③单锚点带
   ...
   - **⑥ 上收**：新增 `ftth_common.cluster_by_y`（锚定语义：带首 y0 锚定、None 跳过），
     ...
     验证：单元等价（300 随机 + 边界）逐位一致；小区甲 4 带 15 条真实标题 × 3 档
   ...
   - **② 旋钮**：`column_consensus_y` 新增 `x_tol`（链式聚类，语义同
     ...
     行为逐位不变；命名避开 count_box_icons.py 同名不同义参数）。实测依据：小区甲
...
### 2026-09-19（八十一·补）：采纳另一会话 8 文件 —— P0 静默失配修复 + 分带自动模式 + 权威文档
   ...
   **r4 管线产物（coverage.json 含 标注侧唯一箱位数/键未识别未比对/楼栋键区间歧义
   ...
   （小区甲全图 plan 模式 rc=0，锚点识别与排除项逐条给因）、read_titleblock
   （小区乙 rc=3 申报制语义正确）。
   **脚本（4）**：
...
### 2026-09-19（八十一）：C10 口径修复 —— 单侧不可读不得判矛盾；图标法层数回退
   **背景（真机实锤，非推演）**：小区甲 r4 pipeline 的 inspect.json 里 C10 FAIL
...
### 2026-09-19（八十）：对照表路径补齐单元粒度纪律 —— A′ 直写楼层 × 23 箱全认领
   **背景**：七十九的「图上箱位直写自证」只补对照表缺席的编号；小区乙余 5 个编号
...
rc 仍 0 —— 实测小区甲 4 图 **84 个箱**、小区乙 **23 个箱**全部归 0。但图上自己就写着
...
- **跨提交回归（本轮补做）**：上轮只做了「同一代码内 legacy ↔ new」对拍，本轮补做
  **跨提交**对拍（`6946373^` vs `6946373`）：确认七十六 除小区乙 6 栋修复外
...
③ 小区乙 1#楼 左界外 67 条业务实体（距边界 100~285 ≈ 半个区间宽）判为独立图区（总图区箱清单），
...
- **收益**：凤鸣朝阳 0 变化；小区甲 a 区 4 band 0 变化（新分支只由「x 命中 ≥2 区间」触发，条件严格）；
  **小区乙 6 栋修复**：`8#楼 0→12 层`、`10#楼 0→12 层`（并补回此前为空的标题）、`7#楼 7→12`、
  `9#楼 7→12`、`11#配套楼 8→4`、`4#配套楼 8→4`（去住宅刻度污染）。
- **七十五 的未决项由此解开**：小区乙配套楼刻度组合 `{B1,1F,2F,7F..10F,WF}` 是「列被拆开」的产物，
...
**未决（报人工，非自动判定）**：① 小区甲 a band2 有 6 条楼层刻度（10F~15F，步长 17550 ≠ 该图
...
- **零回归**：legacy 模式产物与项目**现有** `parsed.json` 逐键比对，凤鸣朝阳 + 小区甲 a 区
  4 个 band **全部差异 0 条**（证明对拍基线可信、改造可逐位还原）。
- **凤鸣朝阳**：新旧 **0 变化**（单带标准图，符合预期）。
- **小区甲 a 区 4 band**：**7 栋楼层表由「空」变「有」**（3#/2# 18 层、6#/7#/9# 15~18 层）。
  根因即共享锚点：标题原文 `1-3号楼综合布线系统图`（技能自身已判定为「并列 2 栋」）
  被展开为同 x 同 y 两个锚点，旧归属下 3#楼恒被 1#楼独占而整片为空。
- **小区乙**：分带生效（2 带：住宅 9 栋 / 配套 2 栋）；**9#楼楼层表 0→7**（区间由 37.6 恢复为
...
**未决（报人工，非自动判定）**：小区乙配套楼的楼层刻度组合（`{B1,1F,2F,7F..10F,WF}`）与
...
| 项目 | 骨架 | 箱清单 | 预检结论 |
|---|---|---|---|
| 小区甲 band1 | present(8) | fx_locations(10) | `pending`：`箱清单有·骨架无` 2 项（1号楼1/2单元） |
| 小区甲 band3 | present(12) | fxmap+fx_locations(14) | `pending`：双向 2 + 4 项 |
| 小区甲 band4 | present(6) | fx_locations(6) | `settled`（两侧一致） |
| 小区乙 | **无文件** | fxmap(13 键，另 2 条无单元号) | `unresolved` |
...
4. **体量预算闸门（收紧）** —— 预警线 **47,000 B**（触及即新增默认进 `references/`）／
   ...
   **净增为零原则**（每加一条须同时外移等量旧内容）。
   ...
   - **个案形态列已移出「三要素」表**（原含小区甲 / 凤鸣朝阳 / 小区乙三个项目名），遵循「个案细节不进技能」。
...
**背景**：两会话实测：单图任务（凤鸣朝阳）自造 2 个脚本即交付；多图任务（小区甲，2 图 8 地块）自造 **9 个**
...
**背景**：2026-09-16 小区甲/凤鸣朝阳两会话坑清单（九层归因）定位的三类**静默缺陷**：L3 正则无捕获组解析期 IndexError 裸崩、L2 `--config` 键被静默忽略（全天 42 处/10 会话无一人处理）、L4 自适应容差失准静默丢整地块（12 栋，rc=0）+ 单元标注「最近者胜」无排他约束串楼后被编号去重盖住。本批**不改任何判据本身**，只把静默失败变成响亮失败；成功路径输出经回归验证与改动前逐字节一致。
...
- `read_titleblock_households.py`：
  ...
  - **R4b 重复单元标签**：某楼某标签次数 **> 本图基准绘制次数** 才报（按众数归一化，正常重复绘制不误报；实测小区甲 `3 4号楼 1单元×4` vs 基准×2 精准命中，37 条噪声归零）。
  - 质量报告新增 5 键：单元归属歧义标注数/明细、重复单元标签的楼、单元标注落到无主层户上的数、零数据楼。

**回归验证**（小区甲 1-4 图自适应/显式 + 凤鸣朝阳 BZ 层，三例）
...
- 小区甲1-4：贴末端 **449** 个 → 归层合计 **0**、未归属 **0**、无刻度列 27/27、**rc=0 有产物**
- 小区甲5-9：贴末端 **356** 个 → 归层合计 **0**、未归属 **0**、无刻度列 22/22、**rc=0 有产物**
- 小区乙（对照）：331 → 331，正常
...
- 小区乙：rc=0 不变，产物 **sha 逐字节一致**（`eb6907b61471`）；**逐键对拍 4031 个叶子键，差异 0**
- 小区甲1-4 / 小区甲5-9：rc 0 → **2**（由「静默 0 户」转为显式拦断）
- 凤鸣朝阳 / 繁华里：rc=2 不变（在更早的门禁处中止）
- `ftth.py count-box` 子命令 rc 透传正常（小区甲5-9 rc=2；小区乙 rc=0 有产物）
...
3. **六类元素无统一台账（P1）**：`EQUIP-箱柜` 层上块属性直接写着 `A=家居配线箱`
   （小区乙 443 / 小区甲1-4 449 / 小区甲5-9 356），是**免几何推断**的户数判据，但此前只在画像侧用到、无生产侧台账；
   小区乙 `智-楼层线`（331 条水平段）是 `floor_scale` 的**独立几何来源**，亦未被利用。
4. **分纤箱编号有假阳性（P1）**：关键词扫描把「说明：…在分纤箱盘留5米」「图例 新建分纤箱」一并计为编号
   （实测小区甲 270 条命中中大量属此类），会把箱数抬高一个量级。
...
- 首版把「共享标题展开的多栋」（锚点 x 相同）误报为**切分错误**：实测该情形占全部重叠的 **100%**
  （小区甲5-9 三组 8 栋 / 小区乙 3 栋 / 小区甲1-4 2 栋 / 繁华里 1#·3#）。判据改**分型**：
  `共享锚点重叠` 登记为「须另行核定、非缺陷」，`异锚点同行带重叠` 才计须修。

**回归（多项目 × 多方向）**

- 台账五图（小区乙 143.5MB / 凤鸣朝阳 / 小区甲1-4 / 小区甲5-9 / 繁华里）**全部 rc=0**；
  坐标缺失 **0** / 单元越界 **0** / 必须修 **0**；凤鸣朝阳 7 栋 12 单元边界完整、小区乙 44 条楼层线入账。
- `parse` 改前（备份）vs 改后**逐键对拍**：凤鸣朝阳 17 处差异**全部是新增的 `分纤箱.x`**，
  剔除该键后**逐键全同（差异 0）**；小区甲5-9 **0 差异**。即「修好 a、没坏 b」。
...
- **验收（双侧，可复现）**：
  - **实跑 A/B**（A =（四十二）前备份，B = 当前）：小区甲5-9 `count-box`、凤鸣朝阳 `coverage-vshape`
    在（四十二）版为 rc=1 且无产物；修后 rc=0，产物与 A 版**逐字节 sha 相同**
    （`a45a37e04972…` / `86c12664f324…`）—— 既证明修好，也证明未改坏。
  - `小区甲5-9` 的 `extract_fx_locations`：A 版 `FileNotFoundError`（3 层目录未建）→ 修后 rc=0 且写出产物。
...
> 依据：并行两会话（凤鸣朝阳 / 小区乙）执行全过程监控 + 五项目回归校验。修复原则：
...
- **P0-2 输出目录不建**（8 个脚本）：`analyze_coverage` / `analyze_coverage_vshape` /
  ...
  **与既有 5 处已正确者（merge_json / split_units / gen_addressbook / plan_methods /
  ...
  实测：3 个项目（凤鸣朝阳 / 小区甲5-9 / 繁华里）`--out` 指向 3 层不存在目录，均 rc=0 + 产物落地。
...
- **P0-4 inspect 空集合假 PASS**（`inspect_closure.py`）：`boxes=[]` ⇒ `null_boxes=[]` ⇒ 走 else 报
  ...
  实测（小区乙图）：旧版 rc=0「无 FAIL，可进入后续自检」；新版 rc=2「存在 FAIL，不得出表」。
...
- **补丁后选法（生产路径 `build_steps()` 实测，14 组信号矩阵）**：
  ...
  - **有楼层刻度时逐项不变**：小区乙仍选图标法、小区甲型（无连线层）与凤鸣朝阳（variant 长句）仍直读。
...
- **回归（三项，均与改动前逐项一致）**：
  ...
  ② 小区乙画像对拍 4 行差异**全为文案**（`前置否决` + `cross_check`），选定方法仍为图标法、门禁 pass；
  ③ 三图回归 —— 小区乙（present + 有连线层）→ 图标法；小区甲1-4（present + 无连线层）→ 直读『X户』；
...
- **三图回归（最终证据）**：小区乙（`present` + 有专用连线层）→ **图标法**；小区甲1-4
...
**来源**：小区乙项目会话（22:56→00:01，65 min，产出 386 户标准地址表）全量复盘 + 独立核验。
...
| 改动 | 回归方式 | 结果 |
|---|---|---|
| 皮线去重 | 小区乙全图重跑 vs 改前 JSON | **仅 6#楼变**（68→64 条，15F 8→4）；其余 10 栋逐项一致；修正后与该图**图标法口径一致**（6#楼 64 户） |
| 出表守门 | 小区乙重跑两路径 | 尺度锚污染 → rc=1 并打出可执行提示；局部栋缺标注按 `_soft` 放行不阻断；`--allow-lossy` 正常写出 |
...
| `floor_mark_conflicts` | 合成用例 6 条（含 B1~B9 混入 / 同名多址 / 次序倒挂 / 同址重复不误报 / WF 哨兵不误报）+ 小区乙真实数据 | 合成用例全过；小区乙命中 `4#配套楼`、`7#楼`（见下方"新发现"） |

**本次新发现（已取证，未改动核心逻辑，待裁决）**

- **`分带` 只改锚点网格，文字归属仍按 x —— 跨带泄漏未消除（P0 级结构问题）**。
  证据：`count_households.py` 的文字归属是 `if xmin <= t["x"] <= xmax`（**纯 x 窗口，无 y 分带过滤**），
  分带只影响 `.get("楼")` 的 x 锚点网格。实测小区乙（住宅行 y≈-4036 与配套行 y≈-3621.7 的
...
**触发**：小区甲项目复核 TeleAgent 报出的问题，实测挖出四处同族缺陷。它们共享一个特征——
...
**触发**：小区甲 8 号地 1 号楼 P 列箱号上下颠倒（用户报错）。
...
具体溯源见 `liuxinzhuang-run/小区甲8区1号楼_箱号上下颠倒_溯源_20260914.md`。）

### 2026-09-14（二十）：V 型法生产引擎 `analyze_coverage_vshape.py` 多图幅适配 —— 5 处真实缺陷修复（含"分段错一层"）

**背景**：以小区甲 8 区（恒盛名邸 8 区）1 号楼做「三要素」走查复核时，用技能自带引擎
`analyze_coverage_vshape.py` 复跑，发现它对**一张 DXF 含多个系统图幅**的情形存在 5 处缺陷
（修订前的脚本版本按「总则 · 维护约定」另存备份，不在技能目录内。）

| # | 缺陷 | 现象 | 修复 |
|---|---|---|---|
| 1 | **多图幅未分带** | 一张图含多个系统图块、x 区间完全重叠（小区甲 5/8/9 三区分居不同 y 带），仅按 x 切窗会把不同图幅数据混在一起 | 先按**标题 y 聚类分带**，带内再按 x 切窗；取数同时约束 x 与 y（`w['ylo']<=y<=w['yhi']`）。边界用相邻带中点推——**不能用固定 margin**（系统图数据在标题上方约 +2 万~+34 万） |
...
→ **原规则只在"两箱层距为偶数"时出错，小区甲全项目恰好只有 8 区 1 号楼这一处**，也解释了它为何唯一。
...
**现象**：小区甲 1-4 地块图纸有 25 列 × 18 条米数标注（写法 `1Px2芯x24m`），
...
用户指出：**方法名不该绑死在 `HDD` 上** —— 小区甲图上该图标的图内文字就是 `HD`
...
- **新增前置条件（本法最大盲区）**：图标必须是**「每户一个」**。小区甲 1-4/5-9 两图实测：
...
- 以上三条均由小区甲项目独立复跑得出（5-9 出表 677/730/963 行、486/516/676 户，与本文档 §五 实测记录逐值相同）。
...
① 会重复绘制（实测小区乙 335 → 331，且"总数==总数"式守恒校验抓不到）；
...
此前住宅楼户数走「系统图 vs 平面图 HDD」交叉验证，实测（小区乙）两者口径差一倍时仍需人工回图裁决，
...
| # | 事项 | 处置 | 验证结果 |
|---|---|---|---|
| 1 | bldg-pattern 漏匹配防护 | `extract_fx_map.py` 运行时断言：区域内有「配套/商业/附属」文字但 bldg-pattern 零命中 ⇒ WARNING 提示 pattern 漏匹配（防 FX22/FX23 误归邻楼根因复发） | 小区乙 pattern 已含「配套」→ 不误报；FX22→4#配套楼-1F、FX23→11#配套楼1F 归属正确 |
| 2 | LWIRE 漏网 | `RE_MEP_LAYER` 补 `lwire`（机电走线层名前缀，旧版排除清单有、重写时丢失） | 三图回归不回归 |
| 3 | 小区甲伪编号 | `judge_fx_overview_map` 新增判据：**编号 x 跨度占图纸主体 >50% ⇒ 散布全图、不构成集中总图 ⇒ absent**（阈值实测完美分隔：小区甲 54%/62%、凤鸣朝阳 75% vs 小区乙 4%） | 小区甲 1-4（252 条/54%）、5-9（216 条/62%）variant→**absent**；小区乙 present 不回归 |
...
| 补丁18 | **词表纠偏**：「光分路箱」误入锚点词表 | 补丁 15 加「光分路箱」过头——实测该词命中的全是 **576 芯落地式光分路箱（ODF/光交设备）**，非 FTTH 楼内分纤箱（小区乙 6 处命中：3 处在区域外 x≈28000、3 处在图签设备表区）；从 `RE_FX_ANCHOR_KW` 移除，保留「配线箱」 | 小区乙锚点 26→23、覆盖率 91%→**100%**；小区甲锚点 58/42 不变（其命中词为「配线箱」） |

**四图回归（补丁 17+18 后，均 rc=0）**

| 图纸 | plan 关键结果 |
|---|---|
| 小区乙 | 锚点 23→有效 21→**21/21=100%**；fx_overview present；候选层 2；门禁 PASS |
| 凤鸣朝阳 | absent 不回归；锚点 17（fallback+WARNING）；门禁 PASS |
| 小区甲 1-4 / 5-9 | fx_overview **absent**（伪编号判据生效）；锚点 58/42；门禁 PASS、EXIT=0 |
...
**经验沉淀**
- 锚点词表每加一个词，必须拿**全部历史图纸**回归——同一词汇在不同图纸指代不同设备（「光分路箱」在小区甲语境≈分纤箱俗名候选，在小区乙语境=ODF 光交设备）；词表只收**当前已实测为真分纤箱**的词。
...
| # | 事项 | 处置 | 验证结果 |
|---|---|---|---|
| P0-1 | 安装楼层「直写优先」落地 | **代码已具备**（口径A直写优先、冲突入待裁决），此前从未用新代码重跑验证 | 小区乙重跑：初版 23 箱全冲突（箱在配纤表坐标）；接箱符号层后 **21 箱一致 + 2 箱冲突显式列出**，直写值全部保住 |
| P0-2 | 机电专业黑名单 | 新增 `RE_MEP_LAYER`（弱电其他/安防/照明/动力/接地/桥架等，`wire_layer_candidates` 命中即排除） | 小区乙候选层 `RX-wire.通讯+WIRE-弱电其他(3层)` → `RX-wire.通讯+WIRE-通讯(2层)`；凤鸣朝阳 absent 不回归 |
| P0-3 | 第二步认不到箱图形符号 | 独立探针定位：小区乙箱符号在 **`R-设备.通讯`** 层（21 个闭合小矩形 11.6×4.6，同层无杂线）；`analyze_coverage.py --fx-symbol-layer` 接入 | 23/23 箱位置来源从「编号文字（回退）」→ **图形符号**；**覆盖范围首次全量落地**（FX01#=B1~9F/FX02#=10F~17F，与既有真值核对一致） |
| P0-4 | `intake_table` rglob 吃产物目录 | rglob 遍历时排除 `run_*/_diag/backup` 等产物目录 + 本技能输出文件名前缀 | 小区乙（根目录无表格）absent ✓；小区甲（根目录 4 张真实地址表）variant ✓；凤鸣朝阳（1 张）variant ✓ |
...
| 补丁15 | 锚点词表缺「配线箱」 | `RE_FX_ANCHOR_KW` 补「配线箱|光分路箱」（小区甲系统图箱标注写作『配线箱』；与『配电箱』字符不重叠） | 小区甲锚点 0 → **58 个**（TEL_TEXT 层） |

**三图回归（同一命令族，均 rc=0）**

| 图纸 | plan 关键结果 |
|---|---|
| 小区乙（共享混合型） | 锚点 23→有效 21→**配对 21/21=100%**；候选层 2 个（黑名单生效）；intake absent；门禁 PASS |
| 凤鸣朝阳（楼-簇组织型） | `vertical_bus_traceable=absent` **不回归**（无通讯层）；intake variant（真实表格）；门禁 PASS |
| 小区甲 1-4/5-9（测绘坐标系新形态） | 锚点 58 个（配线箱词表生效）；floor_scale present（36 层刻度）；hdd 449 个；竖线法 absent → 覆盖选 V 型法（同级选法，非降级）；**intake variant（4 张真实表格）**；门禁 PASS、申报 ✔ |
...
- 产物（小区乙 `run_20260913/`）：`fx_map.json`（23 编号全口径A）、`cov_full.json`（23 箱全定位+全量覆盖+冲突 2 项+裁决 8 项）。

**新暴露（未修，转下轮）**
- 小区甲 `fx_overview_map=variant`：`FX\d+#?` 在 TEL_TEXT/GCD_HC 命中 252 条散布全图的伪编号（该图无真实 FX 编号，编号 0 条），判据需加「散布全图 ⇒ absent」的形态判定。
...
> 实测（小区乙图 150MB）：23 箱中 **21 箱冲突**（例：FX01# 总图直写 5F / 区间法 WF）。
...
- **回归（小区乙图）**：23 箱 **21 冲突 / 2 一致**（FX18# 5F、FX23# 1F 一致）；
...
## 六十六（2026-09-18 小区乙图三轮迭代实跑：4 处 P0/P1 修复）

场景：`小区乙.dxf`（11 栋含 2 配套楼；7#/8#/10# 三栋共享一张系统图；全图无 `N层/M户` 图签
...
- **修（P0）：`gen_addressbook.py` NameError** —— `_fx_pmap`/`_norm_fx` 定义块原先嵌在
  ...
  不存在，而模块级 `gen_unit_rows` 引用它必然 NameError（实测小区乙 386 户出表即炸）。
  修法：定义块上移模块级；归一化**应用**块仍在 coverage 加载后（无 coverage 无需归一）。
  备份：`Desktop/小区乙/.temp/backup_20260919_1319_pre_fix_normfx/`。
...
## 2026-09-19 小区乙第 3 轮（刻度列误合并 P0 修复）
...
- **修（P0）：`count_box_icons.py` 的 `_scales_from_rows` 合并逻辑吞掉独立刻度列** ——
  ...
  备份：`Desktop/小区乙/.temp/backup_20260919_1339_count_box_merge_fix/`（sha1 一致）。
- 同类模式全量排查：`analyze_coverage_vshape.py` 的合并基于「索引相邻 + 同值」、
  `ftth_batch` / `merge_json` 为 JSON 合并 —— **唯一中招点即此一处**。

## 2026-09-19 小区乙第 4 轮（偏移异常判据改为「主偏移整数倍」）
