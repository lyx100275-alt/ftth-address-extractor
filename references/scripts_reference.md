...
| `parse_dxf_structured.py` | 结构化解析：楼栋/单元/分纤箱/楼层表/INSERT | `--text-layer --text-type --title-pattern --floor-pattern --hu-pattern --cable-pattern --cable-keywords --fx-pattern --unit-pattern --unit-cluster --unit-range --y-tol --insert-blocks --insert-attrib-tag --insert-attrib-val` |
