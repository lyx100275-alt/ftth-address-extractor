...
> 图内文字**没有统一写法**，不得写死某一种：实测云峰写 `HDD`、柳辛庄写 `HD`
...
实测（云峰项目，331 户）该偏移恒定：4.04 / 4.03 / 4.13 / 4.37 四档，331/331 命中，
...
- **图标必须是「每户一个」——这是本法的前置条件（2026-09-14 柳辛庄实测新增）**：
  ...
  柳辛庄 5-9 实测：15 列共 356 个图标，列内 Δy 全为 15600/17550（＝层高），
...
def diagnose_no_bond(cands, search_radius):
    """打印「候选与皮线不相邻」的诊断（2026-09-14 柳辛庄实测新增，两个门禁共用）。
    ...
    但两者就是不相邻时，必须能区分两种根因，否则会误判成"参数没调对"而反复空跑：
      ...
      ② 本图图标根本不是「每户一个」，而是「每单元每层一个」的示意画法
         （柳辛庄即此类：图标配 `x2`/`x3` 乘数标注，且图上无皮线图元）。
    """
