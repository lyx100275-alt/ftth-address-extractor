...
#   中间楼栋**整栋丢失**（实测小区甲 band1 丢 2#楼、band3 丢 5#楼；图上另有
...
def find_bldg_anchors(texts, title_re, log=None, expand_ranges=None):
    ...
    2026-09-15 修正（P0 接线缺陷）：`parse_bldg_nums_ex` 早就返回 `ambiguous`
      ...
      规则写了没接线。实测后果：小区甲标题「1-3号楼综合布线系统图」静默取到
    ...
    for t in texts:
        ...
        #   实测小区甲：不归一得 14 个锚点（含 1#楼/1号楼 等 5 组重名），归一后 9 个。
        for num in nums:
