...
Step 1a 向解析阶段传递画像，**至少**回答两项：**① 分纤箱总图**（有没有？编号可读否？安装位置＝楼栋/单元/安装楼层是否同图可读）、**② 系统图选法**（分纤箱位置 / 覆盖范围 / 每层户数各用什么方法统计——**只申报方法，结果由第二步跑出**）。**这是申报制，不是存在性检查**：没有总图、只有系统图的图纸**也得过**；有就说有、没有就说没有，均附证据。
...
### C4 判定契约：总图判据（2026-09-13 收紧）

编号须与楼栋/单元标注**同行**才算总图（对照表形态）：
...
## 图纸信息模型：三要素（2026-09-14 用户确立，最顶层）

> **一张 FTTH 竣工图上，需要读的只有三样东西。三样读全，标准地址表即可直接推出，不需要任何额外的工程判断。**

| # | 要素 | 回答什么问题 |
|---|---|---|
| ① | **分纤箱编号 + 安装位置** | 这个箱是谁、装在哪层 |
| ② | **分纤箱覆盖楼层** | 这个箱管哪些层（**首选直读**；否则由 ②-a 皮线米数 V 型 或 ②-b 竖干断口推出） |
| ③ | **每层户数** | 每层展开几行 |
...
- **三要素必须逐栋 × 逐单元读全**，缺一不可。缺少任何一项，先回到图纸找它的**其他承载形式**（换图层、换写法、换实体类型），**不得因为"这个写法没匹配上"就判它不存在**（配合「镜像铁律」，要求 absent 项必须自证）。
- **优先用直读**：能直读的绝不推导；必须推导时跑方法池（并跑互证见 **L1-C1**）。
- **引用纪律**：技能内文档/代码互相引用一律用**章节名或条目标题**（层内条目用 `L0-I#` / `L1-C#`），**禁止写行号**——行号随任何插入而失效，留下的是会静默指错位置的"死引用"。
- **读全后先做一次"三要素体检"**：每栋每单元是否 ① 有箱号+安装层、② 有覆盖层结论、③ 有每层户数；三项齐全才允许生成表。
- **同一要素多处出现时必须校验一致性**：**箱体编号、安装位置、每层户数**若在图纸标注/平面图/系统图/设计院对应表/采集表**多处出现**，**必须校验各处表述是否统一；前后矛盾按 §五 分级处置**，不得静默择一、不得脚本自动取舍、**不得自行推理断定哪一方正确**。
...
探查不只产出参数，还必须产出**图纸画像**——即"这张图各步骤该用哪个方法"。这就是「探查是测量的指挥：自己不测量，只定怎么测」的落地。
